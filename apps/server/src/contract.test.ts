...
test('v11 contract manifest is deterministic and covers the 69 public tools', async () => {
  ...
  try {
    ...
    assert.equal(first.tools.length, 69);
    assert.equal(first.fingerprint, second.fingerprint);
    assert.match(first.fingerprint, /^[a-f0-9]{64}$/);
    assert.equal(new Set(first.tools.map((tool) => tool.name)).size, 69);
  } finally {
